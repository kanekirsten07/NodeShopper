...
var id;
	...
	response.write('</div>');
	response.write('<script src="//ajax.googleapis.com/ajax/libs/jquery/1.8.1/jquery.min.js"></script>')
	response.write('<script type="text/javascript"> $(".remove").click(function(e){$.ajax({type:"POST", url: "/removegroceries", data:{id:e.currentTarget.value}, success: function(){var cid = e.currentTarget.value; var row = "#row" + cid; $(row).hide("slow");}});});$(".edit").click(function(e){var a, b, c, d, e, f, g, h; var cid = e.currentTarget.value; var row = "#row" + cid; $(row+" td").each(function(cid, index) {if(index ===0){a = $(this).html()}else if(index ===1){b =$(this).html()}else if(index ===2){c=$(this).html()}else if(index===3){d=$(this).html()}else if(index===4){e=$(this).html()}else if(index===5){f=$(this).html()}else if(index===6){g=$(this).html()}}); $(row).html("<input type=text id=itemname></input><input type=text id=datepurchased></input><input type=text id=taxable></input><input type=text id=paidfor></input><input type=text id=purchasername></input><input type=text id=shared></input><input type=text id=price></input><button value=cid class=doneediting>Done</button>"); $("#itemname").val(a);$("#datepurchased").val(b);$("#taxable").val(c); $("#paidfor").val(d); $("#purchasername").val(e); $("#shared").val(f); $("#price").val(g); $(".doneediting").val(cid); $(".doneediting").click(function(e){alert(e.currentTarget.value);});}); function deletediv() { var d = document.getElementById("groceries"); d.parentNode.removeChild(d);} </script>');
	response.write('</body></html>');
	}
	response.end();
    });
